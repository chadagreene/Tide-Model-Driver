% This file reads the old binary AODTM5 tide model and converts it to NetCDF. 
% 
% This script calls some legacy functions from TMD2.5 to load the old data.
% 
% Written by Chad A. Greene, NASA/JPL, May 2022. 
% 
% This script takes about 2 minutes to run on my 2019-era laptop. 

addpath(genpath('/Users/cgreene/Documents/MATLAB/TMD3.00_alpha'))
addpath(genpath('/Users/cgreene/Documents/data/tides/AODTM5'))

%% Enter initial file info 

% Output file: 
newfilename = ['/Users/cgreene/Documents/data/tides/AODTM5.nc']; 

% Input files: 
filename_grd = '/Users/cgreene/Documents/data/tides/AODTM5/grid_Arc5km'; 
filename_h = '/Users/cgreene/Documents/data/tides/AODTM5/h0_Arc5km.oce'; 
filename_u = '/Users/cgreene/Documents/data/tides/AODTM5/UV0_Arc5km'; 

res = 5; % (km) input grid resolution 

con_string = 'm2 s2 n2 k2 k1 o1 p1 q1'; % constituents in original model order, obtained by rd_con('h0_Arc5km.oce') 
...
wct = int16(interp2(x,y,wct,Xi,Yi,'linear',32767)); 
...
for k = 1:Ncons
    tmp = hRe(:,:,k); 
    tmp(mask==2) = FillValue; % mask==2 for h, but mask~=1 for U and V  
    hRe(:,:,k) = tmp; 

    tmp = hIm(:,:,k); 
    tmp(mask==2) = FillValue;
    hIm(:,:,k) = tmp; 
...
netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'),'Title','AODTM5');
netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'),'Description','AODTM5: Arctic Ocean Dynamics-based Tide Model, on 5 km grid, developed in 2004.');
...
netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'),'Data_citation','Padman L , Erofeeva S , and Howard S. 2020. AODTM5: Arctic Ocean Dynamics-based Tide Model, on 5 kilometer grid, developed in 2004. DOI information is available at https://arcticdata.io/.')
...
netcdf.putAtt(ncid,hRe_var_id,'_FillValue',    FillValue);
...
netcdf.putAtt(ncid,hIm_var_id,'_FillValue',    FillValue);
...
netcdf.putAtt(ncid,wct_var_id,'_FillValue',    int16(32767));
...
netcdf.putVar(ncid,vRe_var_id,ipermute(VIm,[2 1 3]));
netcdf.putVar(ncid,vIm_var_id,ipermute(VRe,[2 1 3]));
